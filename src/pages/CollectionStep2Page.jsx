...
export default function CollectionStep2Page() {
  ...
  const handleSubmit = async () => {
    for (const site of record.sites) {
      ...
      if (total !== site.bag_count) { setError(`${site.site_name}의 배분 수량(${total}자루)이 수거량(${site.bag_count}자루)과 다릅니다`); return }
    }
    setSubmitting(true)
    setError('')
    try {
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#F9FAFB', display: 'flex', flexDirection: 'column' }}>
        ...
        {record?.sites.map(site => {
          ...
          return (
            <div key={site.detail_id} style={{ marginBottom: '32px' }}>
              <div style={{ backgroundColor: '#EFF6FF', borderRadius: '14px', border: `2px solid ${over ? '#FCA5A5' : '#BFDBFE'}`, padding: '20px 24px', marginBottom: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                ...
                <div style={{ textAlign: 'right' }}>
                  ...
                  {siteTotal > 0 && <p style={{ margin: '2px 0 0', fontSize: '13px', fontWeight: 600, color: over ? '#EF4444' : '#10B981' }}>배분 {siteTotal}자루 {over ? '⚠ 초과!' : siteTotal === site.bag_count ? '✓ 완료' : ''}</p>}
                </div>
              </div>
              {over && <p style={{ color: '#EF4444', fontSize: '13px', fontWeight: 600, margin: '0 0 12px', padding: '8px 12px', backgroundColor: '#FEF2F2', borderRadius: '8px', border: '1px solid #FCA5A5' }}>⚠ 배분 수량({siteTotal}자루)이 수거량({site.bag_count}자루)을 초과했습니다</p>}
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, backgroundColor: '#fff', borderTop: '1px solid #F3F4F6', padding: '16px 32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', zIndex: 10 }}>
        ...
        <span style={{ fontSize: '14px', fontWeight: 600, color: hasOverflow ? '#EF4444' : '#6B7280' }}>총 {totalAll}자루 {hasOverflow ? '⚠ 초과' : ''}</span>
