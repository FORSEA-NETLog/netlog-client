...
export default function CollectionStep2Page() {
  ...
  const getSiteTotal = (detailId) =>
    Object.values(rackInputs[detailId] || {}).reduce((sum, v) => sum + (parseInt(v) || 0), 0)

  const isOverTotal = (site) => getSiteTotal(site.detail_id) > site.bag_count

  const totalAll = record?.sites.reduce((sum, s) => sum + getSiteTotal(s.detail_id), 0) || 0
  const hasOverflow = record?.sites.some(s => isOverTotal(s)) || false
  const canSubmit = totalAll > 0 && !hasOverflow && !submitting

  const handleRackInput = (detailId, rackCode, value) => {
    setRackInputs(prev => ({
      ...prev,
      [detailId]: { ...prev[detailId], [rackCode]: value }
    }))
  }

  const handleSubmit = async () => {
    for (const site of record.sites) {
      const total = getSiteTotal(site.detail_id)
      if (total !== site.bag_count) {
        setError(`${site.site_name}의 배분 수량(${total}자루)이 수거량(${site.bag_count}자루)과 다릅니다`)
        return
      }
    }
    setSubmitting(true)
    ...
    try {
      const sites = record.sites.map(site => ({
        detail_id: site.detail_id,
        site_id: site.site_id,
        racks: Object.entries(rackInputs[site.detail_id] || {})
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#F9FAFB', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, maxWidth: '900px', width: '100%', margin: '0 auto', padding: '0 32px 120px' }}>

        <Stepper current={2} />

        {record?.sites.map(site => {
          const siteTotal = getSiteTotal(site.detail_id)
          const over = isOverTotal(site)
          ...
          return (
            <div key={site.detail_id} style={{ marginBottom: '32px' }}>
              ...
              <div style={{
                ...
                border: `2px solid ${over ? '#FCA5A5' : '#BFDBFE'}`,
                ...
                <div>
                  ...
                  <p style={{ margin: '4px 0 0', fontSize: '13px', color: '#6B7280' }}>
                    {site.site_code}{site.address ? ` · ${site.address}` : ''}
                  </p>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <span style={{ fontSize: '20px', fontWeight: 700, color: '#EF4444' }}>{site.bag_count}자루</span>
                  {siteTotal > 0 && (
                    <p style={{
                      margin: '2px 0 0', fontSize: '13px', fontWeight: 600,
                      color: over ? '#EF4444' : '#10B981'
                    }}>
                      배분 {siteTotal}자루 {over ? '⚠ 초과!' : siteTotal === site.bag_count ? '✓ 완료' : ''}
                    </p>
                  )}
                </div>
              </div>
              ...
              {over && (
                <p style={{
                  ...
                  ⚠ 배분 수량({siteTotal}자루)이 수거량({site.bag_count}자루)을 초과했습니다
                </p>
              )}
                ...
                {RACK_CODES.map(code => {
                  ...
                  return (
                    <div key={code} style={{
                      ...
                      border: `1.5px solid ${isRackOver ? '#FCA5A5' : hasInput ? '#BFDBFE' : '#F3F4F6'}`,
                      padding: '20px'
                    }}>
                      {/* 헤더 */}
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                          <div style={{
                            ...
                            backgroundColor: hasInput ? '#2563EB' : '#F3F4F6',
                            color: hasInput ? '#fff' : '#6B7280',
                            display: 'flex', alignItems: 'center', justifyContent: 'center',
                          ...
                          <div>
                            ...
                            <p style={{ margin: 0, fontSize: '12px', color: '#9CA3AF' }}>
                              {rack.current_count}/{rack.max_capacity}자루 적재 중
                            </p>
                          </div>
                        </div>
                        {hasInput && (
                          <span style={{
                            ...
                            color: isRackOver ? '#EF4444' : '#2563EB'
                          }}>
                            → 합계 {afterCount}자루
                          </span>
                        )}
                      ...
                      <div style={{
                        ...
                        <div style={{
                          ...
                          backgroundColor: isRackOver ? '#EF4444' : '#2563EB',
                          width: `${afterPct}%`,
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        ...
                        <div style={{ position: 'relative', flex: 1 }}>
                          <input
                            type="number" min="0"
                            value={rackInputs[site.detail_id]?.[code] || ''}
                            onChange={e => handleRackInput(site.detail_id, code, e.target.value)}
                            style={{
                              width: '100%', padding: '10px 40px 10px 12px',
                              border: `1.5px solid ${isRackOver ? '#FCA5A5' : '#E5E7EB'}`,
                              borderRadius: '10px', fontSize: '15px', outline: 'none',
                              boxSizing: 'border-box',
                              backgroundColor: isRackOver ? '#FEF2F2' : '#fff',
                              color: '#111827'
                            }}
                          />
                          <span style={{
                            position: 'absolute', right: '12px', top: '50%',
                            transform: 'translateY(-50%)', fontSize: '13px', color: '#9CA3AF'
                          }}>자루</span>
                        </div>
                      </div>
                      ...
                      {isRackOver && (
                        <p style={{ margin: '6px 0 0', fontSize: '12px', color: '#EF4444' }}>
                          용량 초과! (최대 {rack.max_capacity - rack.current_count}자루 추가 가능)
                        </p>
                      )}
                    </div>
                  )
      ...
      <div style={{
        ...
        <span style={{
          ...
          color: hasOverflow ? '#EF4444' : '#6B7280'
        }}>
          총 {totalAll}자루 {hasOverflow ? '⚠ 초과' : ''}
        </span>
