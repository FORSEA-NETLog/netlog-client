          ...
          <div key={idx} style={{ display: 'flex', alignItems: 'center' }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px' }}>
              <div style={{
                ...
                backgroundColor: active ? '#2563EB' : done ? '#2563EB' : '#F3F4F6',
            ...
            {idx < steps.length - 1 && (
              <div style={{
                width: '120px', height: '2px', marginBottom: '18px',
...
export default function CollectionStep1Page() {
  ...
  const [siteWeights, setSiteWeights] = useState({}) // { detail_id: weight_kg }
  ...
  const handleSubmit = async () => {
    if (!transferPerson || !vehicleNumber) {
      setError('운반 담당자와 차량 번호를 입력해주세요')
      return
    }
    const hasEmptyWeight = Object.values(siteWeights).some(v => !v || v <= 0)
    if (hasEmptyWeight) {
      setError('모든 집하장의 계근 무게를 입력해주세요')
      return
    }
  ...
  const inputStyle = {
    ...
    backgroundColor: '#fff', boxSizing: 'border-box', color: '#111827'
  }
  const labelStyle = { fontSize: '13px', color: '#6B7280', marginBottom: '6px', display: 'block' }
  const sectionNumStyle = (done) => ({
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#F9FAFB', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, maxWidth: '720px', width: '100%', margin: '0 auto', padding: '0 24px 120px' }}>
        ...
        {record?.sites.map(site => (
          <div key={site.site_id} style={{
            ...
            border: '2px solid #BFDBFE', padding: '16px 20px',
        ...
        <div style={{ marginBottom: '24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px' }}>
            <div style={sectionNumStyle(transferPerson && vehicleNumber)}>
        ...
        {record?.sites.map((site, idx) => (
          <div key={site.detail_id} style={{ marginBottom: '24px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px' }}>
              <div style={sectionNumStyle(siteWeights[site.detail_id] > 0)}>
            ...
            <div style={{ position: 'relative' }}>
              <input
                ...
                type="number"
                placeholder="총 무게를 입력하세요."
                value={siteWeights[site.detail_id] || ''}
                onChange={e => setSiteWeights(prev => ({ ...prev, [site.detail_id]: e.target.value }))}
              />
              <span style={{
      ...
      <div style={{
        ...
        <span style={{ fontSize: '13px', color: '#9CA3AF' }}>
          {completedCount} / {totalSteps} 입력완료
        </span>
