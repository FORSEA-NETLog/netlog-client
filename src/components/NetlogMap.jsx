...
export default function NetlogMap() {
  ...
  useEffect(() => {
    ...
    const triggerLight=(n)=>{const l=dynamicLights[n];if(l?.mat){l.target.copy(l.highlight);if(l.timeout)clearTimeout(l.timeout);l.timeout=setTimeout(()=>l.target.copy(l.base),1000)}}
    ...
    function calculateBaseBounds(){
      ...
      if(sa>TARGET_ASPECT){baseRenderWidth=bw;baseRenderHeight=bw/sa}else{baseRenderHeight=bh;baseRenderWidth=bh*sa}
      updateCameraView()
    }
    ...
    new GLTFLoader().load('/models/netlog_nla_netspa_text.glb',(gltf)=>{
      ...
      const loadClip=(name,sub)=>{
        const clip=THREE.AnimationClip.findByName(clips,name);if(!clip)return null
    ...
    return()=>{
      ...
      window.removeEventListener('mouseup',onMouseUp);window.removeEventListener('wheel',onWheel)
      window.removeEventListener('touchstart',onTouchStart);window.removeEventListener('touchmove',onTouchMove)
      window.removeEventListener('touchend',onTouchEnd);window.removeEventListener('resize',calculateBaseBounds)
      renderer.dispose()
    }
  },[])
